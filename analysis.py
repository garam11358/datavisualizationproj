...
class Analysis:
    ...
    def min(self, headers, rows=[]):
        ...
        '''
        new_data = self.data.select_data(headers,rows)
        return new_data.min(axis = 0)
        pass
    ...
    def mean(self, headers, rows=[]):
        ...
        '''
        new_data = self.data.select_data(headers,rows)
        sum_new_data = new_data.sum(axis = 0)
        return sum_new_data/len(new_data)
        pass
    ...
    def var(self, headers, rows=[]):
        ...
        '''
        new_data = self.data.select_data(headers,rows)
        mean_data = self.mean(headers,rows)
        var_data = (new_data - mean_data)**2
        var_data = var_data.sum(axis = 0)
        return (var_data/(len(new_data) - 1))
        pass
    ...
    def std(self, headers, rows=[]):
        ...
        NOTE: Loops are forbidden!
        '''
        var_data = self.var(headers,rows)
        return var_data**0.5
        pass
    ...
    def scatter(self, ind_var, dep_var, title):
        ...
        '''
        x_var = self.data.select_data([ind_var])
        y_var = self.data.select_data([dep_var])
        x_var = x_var.reshape(len(x_var))
        y_var = y_var.reshape(len(y_var))
        plt.title(title)
        plt.xlabel(ind_var)
        plt.ylabel(dep_var)
    ...
    def pair_plot(self, data_vars, fig_sz=(12, 12), title=''):
        ...
        '''
        plot = self.data.select_data(data_vars)
        fig, axs = plt.subplots(len(data_vars),len(data_vars),sharex = True,sharey = True,figsize=fig_sz)
        fig.suptitle(title)
        for k in range(len(data_vars)):
            for i in range(len(data_vars)):
                axs[k,i].scatter(plot[:,i],plot[:,k])
                if (k == len(data_vars) - 1) and (i == 0):
                    axs[k,i].set(xlabel = data_vars[i], ylabel = data_vars[k])
                elif(k == len(data_vars) - 1):
                    axs[k,i].set(xlabel = data_vars[i])
                elif(i == 0):
                    axs[k,i].set(ylabel = data_vars[k])   
        return (fig,axs)
        pass